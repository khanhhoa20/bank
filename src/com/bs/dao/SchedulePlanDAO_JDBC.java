...
import com.bs.model.SchedulePlan_JDBC;
...
public class SchedulePlanDAO_JDBC {
	...
	public List<SchedulePlan_JDBC> getAllSchedulePlans() {
		List<SchedulePlan_JDBC> list=new ArrayList<>();
		connection=DBUtil.getConnection();
		try {
			ps=connection.prepareStatement("select * from schedule_plan");
			rs=ps.executeQuery();
			while (rs.next()) {
				list.add(new SchedulePlan_JDBC(rs.getLong(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getTimestamp(5), rs.getTimestamp(6), rs.getTimestamp(7)));
			}
	...
	public SchedulePlan_JDBC findSchedulePlanById(long id) {
		SchedulePlan_JDBC schedulePlan=null;
		connection=DBUtil.getConnection();
		try {
			ps=connection.prepareStatement("select * from schedule_plan where id=" +id);
			rs=ps.executeQuery();
			if(rs.next()) {
				schedulePlan=new SchedulePlan_JDBC(rs.getLong(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getTimestamp(5), rs.getTimestamp(6), rs.getTimestamp(7));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public void insertSchedulePlan(SchedulePlan_JDBC s) {
		connection=DBUtil.getConnection();
		try {
